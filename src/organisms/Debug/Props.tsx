...
export const DebugProps = ({ children, ...props }) => {
  const { debug, local } = props;
  const showDebugButton = debug || local;
      ...
      <Modal scrollable size="lg" isOpen={showDebugModal} toggle={toggleDebugModal}>
        ...
        <ModalBody>
          <Card className="DebugProps">
            <CardBody>
              <code>
                <pre>{JSON.stringify(Object.assign(defaultDebug, props), null, 2)}</pre>
              </code>
            </CardBody>
          </Card>
        </ModalBody>
        <ModalFooter>
          <Button color="secondary" onClick={toggleDebugModal}>
            Close
          </Button>
        </ModalFooter>
      </Modal>
